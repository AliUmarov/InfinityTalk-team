...
export const startChat = (mainUser, partner) => {
  ...
  return async (dispatch) => {
    const res = await fetch("https://infinity-talk.herokuapp.com/chats/", {
      method: "POST",
      body: JSON.stringify({
        members
      }),
...
export const loadChats = (mainUserId) => {
  return async (dispatch) => {
    const res = await fetch(`https://infinity-talk.herokuapp.com/chats/user/${mainUserId}`);
...
export const newMessage = (chatId, user, text) => {
  return async (dispatch) => {
    await fetch(`https://infinity-talk.herokuapp.com/chats/${chatId}`, {
      method: "PATCH",
      body: JSON.stringify({
        messages: {text, user}
      }),
